...
const GoalStep: React.FC<GoalStepProps> = ({ step }) => {
  ...
  return <div className="flex items-center justify-between bg-white px-3 py-2 rounded-md border">
    <button onClick={() => updateStep(step.id, {...step, completed: !step.completed})} className="text-blue-500">
      {step.completed ? <FaCheckCircle /> : <FaRegCircle />}
    </button>
    <span className="flex-1 text-gray-700">{step.description}</span>
    <button onClick={() => deleteStep(step.id)} className="text-red-500 hover:text-red-700">
      <FaTrash />
    </button>
  </div>
};
